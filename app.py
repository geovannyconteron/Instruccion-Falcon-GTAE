...
components.html("""
    ...
    function playCockpitClick() {
        ...
        } catch(e) { console.log("Audio demorado o bloqueado"); }
    }
...
# ==============================================================================
# ALARMAS Y ADVERTENCIAS SONORAS TRAS RECARGAS DE MANTENIMIENTO
# ==============================================================================
...
# ==============================================================================
# BARRA LATERAL - CONTROL E INTEGRACIÓN DE PROCEDIMIENTOS RESTRINGIDOS
# ==============================================================================
with st.sidebar:
...
# Inicialización de memorias operacionales de rampa e historial de pasos
...
# Variables de estado de motores para reflejar en el MFD de Streamlit
if "piloto_apu" not in st.session_state: st.session_state.piloto_apu = "OFF"
if "piloto_bleed" not in st.session_state: st.session_state.piloto_bleed = "CLOSED"
if "piloto_eng1" not in st.session_state: st.session_state.piloto_eng1 = "STBY"
if "piloto_lever" not in st.session_state: st.session_state.piloto_lever = "SHUTOFF"
if "piloto_cas" not in st.session_state: st.session_state.piloto_cas = "🟢 SYSTEMS GENERAL RUN NOMINAL"
...
# ------------------------------------------------------------------------------
# MÓDULO III: PROCEDIMIENTOS OPERATIVOS (ENCENDIDO DE MOTORES)
# ------------------------------------------------------------------------------
if opcion_sistema == "MÓDULO III: ENCENDIDO DE MOTORES":
    ...
    st.markdown("<p style='text-align: center; color: #94a3b8;'>Flujo de la Secuencia de Arranque Hidroneumático de los 3 Motores</p>", unsafe_allow_html=True)

    col_interactiva_mandos, col_mfd_honeywell = st.columns([1.2, 1])

    with col_interactiva_mandos:
        ...
        with c_apu[0]:
            if st.button("APU MASTER SWITCH"):
                st.session_state.piloto_apu = "RUN" if st.session_state.piloto_apu == "OFF" else "OFF"
                if st.session_state.piloto_apu == "OFF": st.session_state.piloto_bleed = "CLOSED"
                st.rerun()
            st.markdown(f"<div class='anunciador-verde'>ON (100%)</div>" if st.session_state.piloto_apu == "RUN" else "<div class='anunciador-apagado'>OFF</div>", unsafe_allow_html=True)
        with c_apu[1]:
            if st.button("APU BLEED AIR VALVE"):
                if st.session_state.piloto_apu == "RUN":
                    st.session_state.piloto_bleed = "OPEN" if st.session_state.piloto_bleed == "CLOSED" else "CLOSED"
                st.rerun()
            st.markdown(f"<div class='anunciador-amber'>BLEED OPEN</div>" if st.session_state.piloto_bleed == "OPEN" else "<div class='anunciador-apagado'>CLOSED</div>", unsafe_allow_html=True)
        ...
        c_starts = st.columns(3)
        with c_starts[0]:
            if st.button("ENG 1 START"):
                if st.session_state.piloto_bleed == "OPEN":
                    st.session_state.piloto_eng1 = "CRANK"
                    if st.session_state.piloto_lever == "RUN":
                        st.session_state.piloto_cas = "🚨 ALERT CAS: HOT START IN ENGINE 1!\n Combustible inyectado prematuramente con rotación N2 inferior al 15%."
                        st.session_state.audio_alarma = "alarma_critica"
                else:
                    st.session_state.piloto_cas = "🚨 FALLO DE ARRANQUE EN MOTOR 1: Presión neumática insuficiente (APU BLEED CLOSED)."
                    st.session_state.audio_alarma = "alarma_critica"
                st.rerun()
            st.markdown(f"<div class='anunciador-amber'>{st.session_state.piloto_eng1}</div>" if st.session_state.piloto_eng1 != "STBY" else "<div class='anunciador-apagado'>STBY</div>", unsafe_allow_html=True)
        with c_starts[1]:
            st.button("ENG 2 START", disabled=True)
            st.markdown("<div class='anunciador-apagado'>STBY</div>", unsafe_allow_html=True)
        with c_starts[2]:
            st.button("ENG 3 START", disabled=True)
            st.markdown("<div class='anunciador-apagado'>STBY</div>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)

        # Pedestal Levers
        st.markdown("<div class='subpanel-3d'><div class='titulo-serigrafia'>Pedestal Fuel Control Levers</div>", unsafe_allow_html=True)
        if st.button(f"CORTE DE INYECCIÓN DE COMBUSTIBLE: {st.session_state.piloto_lever}"):
            st.session_state.piloto_lever = "RUN" if st.session_state.piloto_lever == "SHUTOFF" else "SHUTOFF"
            if st.session_state.piloto_lever == "RUN" and st.session_state.piloto_eng1 == "CRANK":
                st.session_state.piloto_eng1 = "RUN NOMINAL"
                st.session_state.piloto_cas = "🟩 SYSTEMS STATUS: ENGINE 1 RUNNING IN IDLE PROFILES"
            elif st.session_state.piloto_lever == "RUN" and st.session_state.piloto_eng1 == "STBY":
                st.session_state.piloto_cas = "🚨 ALERT CAS: HOT START IN ENGINE 1!\n Corte de combustible abierto sin flujo neumático."
                st.session_state.audio_alarma = "alarma_critica"
            st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)

        if st.button("🚨 RESETEAR PARÁMETROS DE ARRANQUE OPERATIVO"):
            st.session_state.piloto_apu = "OFF"
            st.session_state.piloto_bleed = "CLOSED"
            st.session_state.piloto_eng1 = "STBY"
            st.session_state.piloto_lever = "SHUTOFF"
            st.session_state.piloto_cas = "🟢 SYSTEMS GENERAL RUN NOMINAL"
            st.rerun()
        st.markdown("</div>", unsafe_allow_html=True)

    with col_mfd_honeywell:
        st.markdown("### 📺 Honeywell EASy Avionics Display")
        
        # Renderizado dinámico de los tres relojes de aguja analógicos en HTML5 Canvas
        # Cambian al instante y muestran los errores en la caja de texto CAS inferior
        n1_val = 24.2 if st.session_state.piloto_eng1 == "RUN NOMINAL" else 0.0
        n2_val = 57.4 if st.session_state.piloto_eng1 == "RUN NOMINAL" else (25.0 if st.session_state.piloto_eng1 == "CRANK" else 0.0)
        itt_val = 490 if st.session_state.piloto_eng1 == "RUN NOMINAL" else 15

        html_clocks = f"""
        <div style="display:flex; justify-content: space-around; background:#04070e; border:3px solid #334155; padding:15px; border-radius:6px; text-align:center;">
            <div>
                <canvas id="n1_g" width="100" height="100"></canvas>
                <div style="color:white; font-size:0.8rem; font-weight:bold;">N1: {n1_val}%</div>
            </div>
            <div>
                <canvas id="n2_g" width="100" height="100"></canvas>
                <div style="color:white; font-size:0.8rem; font-weight:bold;">N2: {n2_val}%</div>
            </div>
            <div>
                <canvas id="itt_g" width="100" height="100"></canvas>
                <div style="color:white; font-size:0.8rem; font-weight:bold;">ITT: {itt_val}°C</div>
            </div>
        </div>
        <script>
            function d(id, val, max) {{
                let c = document.getElementById(id); if(!c) return;
                let ctx = c.getContext("2d"); ctx.clearRect(0,0,100,100);
                ctx.beginPath(); ctx.arc(50,55,40,0,2*Math.PI); ctx.strokeStyle="#2d3748"; ctx.lineWidth=3; ctx.stroke();
                let a = 0.75*Math.PI + (val/max)*(1.5*Math.PI);
                ctx.beginPath(); ctx.moveTo(50,55); ctx.lineTo(50+30*Math.cos(a), 55+30*Math.sin(a));
                ctx.strokeStyle="#4ade80"; ctx.lineWidth=3; ctx.stroke();
            }}
            d("n1_g", {n1_val}, 100); d("n2_g", {n2_val}, 100); d("itt_g", {itt_val}, 800);
        </script>
        """
        components.html(html_clocks, height=150)

        borde_crt = "#ef4444" if "🚨" in st.session_state.piloto_cas else "#475569"
        fondo_crt = "#200d0d" if "🚨" in st.session_state.piloto_cas else "#000000"
        texto_crt = "#fca5a5" if "🚨" in st.session_state.piloto_cas else "#38bdf8"

        st.markdown(f"""
            <div class="pantalla-mfd" style="border: 5px solid {borde_crt}; background-color: {fondo_crt}; color: {texto_crt}; min-height:340px;">
<div style="border-bottom: 2px solid #334155; padding-bottom: 8px; margin-bottom: 25px; font-weight: bold;"><span>HONEYWELL PDU: AVIONICS SHIELD</span><span>FLIGHT FEED</span></div>
⚙️ STATUS DE COMPRESIÓN HIDRONEUMÁTICA (ATA 70):
 • APU MASTER RECEPTACLE      : {st.session_state.piloto_apu}
 • VALVULA DE PURGA APU BLEED : {st.session_state.piloto_bleed}
 • PEDESTAL FUEL CUT SWITCH   : {st.session_state.piloto_lever}
----------------------------------------------------------------------
🔔 CREW ALERTING SYSTEM (CAS) LIVE DATA FEED:

{st.session_state.piloto_cas}
            </div>
        """, unsafe_allow_html=True)

# ------------------------------------------------------------------------------
# MÓDULOS DE MANTENIMIENTO TÉCNICO (ATA 24 Y ATA 28 INTEGROS RESTAURADOS)
# ------------------------------------------------------------------------------
else:
